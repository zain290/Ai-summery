  ...
  return AGENCY_DATA.services
    .map(
      (service) =>
        ...
        `What we offer: ${service.deliverables.join(', ')}\n` +
...
Rules:
- Only answer questions about ${AGENCY_DATA.name}, its features, supported file types, privacy, and general tool questions.
...
If the user wants to continue the conversation offline, direct them to ${contactEmail}.
  `.trim();
}
